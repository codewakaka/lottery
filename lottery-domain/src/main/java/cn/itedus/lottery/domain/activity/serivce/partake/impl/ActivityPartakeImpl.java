package cn.itedus.lottery.domain.activity.serivce.partake.impl;
...
import cn.itedus.lottery.domain.activity.serivce.partake.BaseActivityPartake;
...
public class ActivityPartakeImpl extends BaseActivityPartake {
    ...
    protected Result checkActivityBill(PartakeReq partake, ActivityBillVO bill) {
        ...
        if (!Constants.ActivityState.DOING.getCode().equals(bill.getState())) {
            ...
            return Result.buildResult(Constants.ResponseCode.UN_ERROR, "活动当前状态不可用");
        }
        //校验，活动日期
    ...
    public Result recordDrawOrder(DrawOrderVO drawOrder) {
        try {
            ...
            return transactionTemplate.execute(status -> {
                try {
                    int lockCount = userTakeActivityRepository.lockTackActivity(drawOrder.getuId(), drawOrder.getStrategyId(), drawOrder.getTakeId());
                    if(0 == lockCount){
                        status.setRollbackOnly();
